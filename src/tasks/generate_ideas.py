...
async def generate_search_ideas(user_description: str, model: str = "gpt-3.5-turbo") -> str:
    ...
    system_prompt = """
        ...
        "abstract": "a research abstract for a potential article that would address the users described problem",
        }
        Example description:
        Current image generation models often struggle with accurately generating
        hands. How could I go about improving these models to fix this?
        Example response:
        {
        "idea": "One solution to improve the accuracy of generated hands in image generation 
        models is to incorporate explicit hand pose supervision into the training process using a 
        pre-trained hand pose estimator. This strategy combines generative diffusion models with auxiliary 
        guidance from a network that specializes in understanding human hand structure. During training, 
        the model is not only penalized for poor visual fidelity but also for generating hand poses that deviate from 
        realistic skeletal configurations, as predicted by the hand pose estimator.",
        ...
        First, reason through how you would solve the problem based on your knowledge.
        Then, generate a solution option. Only return the JSON object
        """
    user_prompt = f"""
